queue().defer(d3.json, '/public/static/gejson/us-states.json').defer(d3.json,
		'/public/static/data/company_id_to_name.json').defer(d3.csv,
		'/public/static/data/indeed.csv').await(visualize);

function visualize(error, statesJson, comID, indeed) {
	if (error) {
		console.log(error);
	}

	vizIndeed(statesJson, comID, indeed);

}

function vizIndeed(statesJson, comID, indeed) {
	...
	var indeedSalaryDim = indeedndx.dimension(function(d) {
		if (+d['salary'] <= 10) {
			...
			return '50-100';
		} else if (+d['salary'] < 200) {
			return '100-200';
		} else if (+d['salary'] < 300) {
			return '200-300';
		} else if (+d['salary'] < 400) {
			return '300-400';
	...
	sd_chart.width(400).height(310).margins({
		top : 10,
		right : 5,
		bottom : 20,
		left : 5
	}).dimension(indeedSalaryDim).group(salaryGr).ordering(function(d) {
		...
		if (d.key == '0-10') {
			...
			return 3;
		} else if (d.key == '100-200') {
			return 4;
		} else if (d.key == '200-300') {
			return 5;
		} else if (d.key == '300-400') {
			return 6;
		} else if (d.key == '400-500') {
			return 7;
		} else if (d.key == '500+') {
			return 8;
		}
		Ï
	}).transitionDuration(500).elasticX(true).xAxis().ticks(4);
	...
	//	
	// top 10 Jobs
	t10j_chart.width(200).height(400).margins({
		top : 10,
		right : 5,
		bottom : 20,
		left : 5
	}).dimension(indeedJobDim).group(Top10JobGr).elasticX(true).cap(10)
			.othersGrouper(null).labelOffsetY(10).xAxis().ticks(4);

	// top 10 Jobs
	t10c_chart.width(200).height(400).margins({
		top : 10,
		right : 5,
		bottom : 20,
		left : 5
	}).dimension(indeedCompanyDim).group(Top10ComGr).cap(10)
			.othersGrouper(null).elasticX(true).labelOffsetY(10).xAxis().ticks(
					4);
	//	
	...
	// ye的美国地图Charts
	indeed_job_map.width(600).height(400).dimension(indeedStateDim).group(
			...
			.projection(d3.geo.albersUsa().scale(600).translate([ 220, 200 ]))
			.title(
					function(p) {
						return "State: " + p["key"] + "\n"
